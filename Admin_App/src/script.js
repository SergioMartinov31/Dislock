...
function renderQuestions() {
    ...
    if (questions.length === 0) {
        ...
        if (isDevMode) autoFillQuestions();
    }
...
function renderAdditionalQuestions() {
    ...
    if (additionalQuestions.length === 0) {
        div.innerHTML = '<p>Все функции выразимы через другие функции в наборе!</p>';
        console.log('No additional questions to display');
    } else {
        additionalQuestions.forEach((q, idx) => {
            div.innerHTML += `
                <div class="coefficient-group">
                    <p>Укажите колонку второй таблицы, доказывающую, что ${q.op} нельзя выразить через ${q.otherOps.join(' и ')}? (Любой из возможных контрпримеров: ${q.columns.join(', ')})</p>
        ...
        if (isDevMode) autoFillAdditionalQuestions();
    }
...
function generateAdditionalQuestions() {
    ...
    console.log('Generated Additional Questions:', additionalQuestions);
    if (additionalQuestions.length > 0) {
...
function checkAdditionalAnswer(idx) {
    ...
    console.log(`checkAdditionalAnswer called with idx=${idx}`);
    ...
    console.log(`Additional Question: ${q.op}, Input: ${input}, Expected: ${q.columns}, isCorrect: ${isCorrect}`);
    if (btn.classList.contains('checked')) return;
    btn.classList.add('checked');
    btn.disabled = true;
...
function autoFillAdditionalQuestions() {
    additionalQuestions.forEach((q, idx) => {
        const input = document.getElementById(`aq_${idx}`);
        if (input) {
            input.value = q.columns[0]; // Берём первый контрпример
        }
    });
}
...
function exportToPDF() {
    ...
    try {
        ...
        // Определяем ширину колонок (делим страницу на две части с учётом отступов)
        const columnWidth = (pageWidth - 40) / 2; // 40 — это сумма левого и правого отступов (20 + 20)
        const leftColumnX = 20; // Начало левой колонки
        const rightColumnX = 20 + columnWidth + 10; // Начало правой колонки (с небольшим отступом 10 между колонками)

        // Собираем данные, введённые пользователем для "Контрпримеров"
        const userQuestions = [];
        questions.forEach((q, idx) => {
            const inputElement = document.getElementById(`q_${idx}`);
            const btn = document.querySelector(`#q_${idx} + button`);
        ...
        // Собираем данные, введённые пользователем для "Контрпримеров выразимости"
        ...
        // Выводим "Контрпримеры" в левой колонке
        ...
        // Выводим "Контрпримеры выразимости" в правой колонке
        ...
        // Определяем максимальную высоту, занятую обеими колонками
